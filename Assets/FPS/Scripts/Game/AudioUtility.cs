...
namespace Unity.FPS.Game
{
    public class AudioUtility
    {
            ...
            float rolloffDistanceMin = 1f, float volume = 1f)
        {
            ...
            impactSfxInstance.name = $"SFX_{clip.name}";
        ...
        public static AudioMixerGroup GetAudioGroup(AudioGroups group)
        {
            ...
            // FIX: Safely catch missing AudioManager instances
            if (s_AudioManager == null)
            {
                Debug.LogWarning($"AudioManager not found in current scene! Cannot load audio group for: {group}");
                return null;
            }
