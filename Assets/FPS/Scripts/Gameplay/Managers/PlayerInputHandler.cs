...
namespace Unity.FPS.Gameplay
{
    public class PlayerInputHandler : NetworkBehaviour
    {
        ...
        {
            // If this instance doesn't belong to the local controller, skip configuration entirely
            if (!IsOwner)
            {
                // Find the PlayerInput component on this specific clone and turn it off
                if (TryGetComponent<PlayerInput>(out PlayerInput externalPlayerInput))
                {
                    externalPlayerInput.enabled = false;
                }

                // Skip the rest of the configuration entirely
                return;
            }
        ...
        void LateUpdate()
        {
            // If we aren't the local player owner, do absolutely nothing here.
            if (!IsOwner) return;
        ...
        {
            // Check if we are the owner first, and safely guard against a missing GameFlowManager or unassigned inputs
            if (!IsOwner || m_FireAction == null) return false;

            // Try to find the GameFlowManager dynamically if it was missed during setup
            if (m_GameFlowManager == null)
            {
                m_GameFlowManager = FindFirstObjectByType<GameFlowManager>();
            }

            bool isGameEnding = m_GameFlowManager != null && m_GameFlowManager.GameIsEnding;

            return Cursor.lockState == CursorLockMode.Locked && !isGameEnding;
        }
