...
namespace Unity.FPS.Gameplay
{
    public class OverheatBehavior : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            AudioMixerGroup targetGroup = AudioUtility.GetAudioGroup(AudioUtility.AudioGroups.WeaponOverheat);
            if (targetGroup != null)
            {
                m_AudioSource.outputAudioMixerGroup = targetGroup;
            }
        }
        ...
        void Update()
        {
            ...
            if (CoolingCellsSound)
            {
                if (!m_AudioSource.isPlaying
                    && currentAmmoRatio != 1
                    && m_Weapon.IsWeaponActive.Value
                    && m_Weapon.IsCooling)
                {
                    m_AudioSource.Play();
                }
                else if (m_AudioSource.isPlaying
                         && (currentAmmoRatio == 1 || !m_Weapon.IsWeaponActive.Value || !m_Weapon.IsCooling))
                {
                    m_AudioSource.Stop();
                    return;
                }
